...
function precisionRounder(val: number, precision: number): number {
...
function solve(stiffnessMatrix: Matrix, forcesVector: Vector): Vector {
  ...
  for (let row = n - 1; row >= 0; row--) {
    ...
    // Calculate the w_i
    w[row] = precisionRounder((F[row] - sum) / K[row][row], 2);
  }
